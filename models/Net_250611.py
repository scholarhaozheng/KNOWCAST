...
class Net_0207(torch.nn.Module):
    ...
    def forward(self, sequences, sequences_y, return_path_prob=False):
        path_prob_list = []      # 用来收集每个 batch 的概率
        section_prob_list = []
        extended_sequences = []
        ...
        for i, data_batch in enumerate(sequences):
            ...
            for j in range(num_graphs):
                ...
                if self.traffic_assignment_included:
                    ...
                    if return_path_prob:  # 只在需要导出时收集
                        # 保证 batch 维度一致，先 unsqueeze(0)
                        path_prob_list.append(
                            OD_path_possibility.unsqueeze(0).detach().cpu()
                        )
                        section_prob_list.append(
                            weighted_T_eff.unsqueeze(0).detach().cpu()
                        )
        ...
        predictions_od = self.decoder_od_do(extended_sequences_y,
                                            ...
                                            edge_attr=edge_attr)
        if return_path_prob:
            path_prob_batch = torch.cat(path_prob_list, dim=0) if path_prob_list else None
            section_prob_batch = torch.cat(section_prob_list, dim=0) if section_prob_list else None
            return predictions_od, path_prob_batch, section_prob_batch
