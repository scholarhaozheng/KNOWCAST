...
def batch_processing(Time_DepartFreDic, OD_path_dic, OD_feature_array_dic,
    ...
    for batch_idx in range(total_batches):
        ...
        partial_processing_function = partial(
            processing_function,
        ...
        results = []
        max_workers = max(1, os.cpu_count()-20 or 1)
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            for result in tqdm(executor.map(partial_processing_function, batch_dict.items()), total=len(batch_dict)):
...
def processing_Time_DepartFreDic_item(Time_DepartFreDic_item, OD_path_dic, station_manager_dict, device):
    rank = 5  # 秩的选择根据数据调整
...
def generating_OD_section_pssblty_sparse_array(base_dir, station_manager_dict_name,
    ...
    if Batch_or_not:
        batch_processing(
            Time_DepartFreDic = Time_DepartFreDic,
            batch_size = 5,
...
Date_and_time_OD_path_cp_factors_dic_file_path = os.path.join(base_dir,
